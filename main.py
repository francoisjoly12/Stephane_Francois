...
projet_Final.conversion_en_fahrenheit = False
projet_Final.resetMinMax = False
...
def connect_to_internet():
    io=None
    try:
        io = projet_Final.connecter_mqtt()
        if io.is_connected:
            io.subscribe("projet-2-Celcius/Farenheit")
            io.subscribe("projet-2-resetTime")
            ntp = adafruit_ntp.NTP(projet_Final.pool, tz_offset=-4)
...
while True:
    ...
    if current_time - last_display_time > 0.5:
        ...
        if projet_Final.conversion_en_fahrenheit:
            temp_actuelle_fahrenheit = projet_Final.celsius_to_fahrenheit(temp_actuelle)
            temp_moyenne_fahrenheit = projet_Final.celsius_to_fahrenheit(temp_moyenne)
            temp_min_fahrenheit = projet_Final.celsius_to_fahrenheit(temp_min)
            temp_max_fahrenheit = projet_Final.celsius_to_fahrenheit(temp_max)
